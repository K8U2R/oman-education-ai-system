...
import { getConfigManager as _getConfigManager } from "./infrastructure/config/core/ConfigManager.js";
...
`);
  ...
  logger.info(
    `   ✅ Database: ${ENV_CONFIG.DATABASE_URL ? "Configured" : "❌ Not Configured"}`,
  );
  ...
  try {
    if (ENV_CONFIG.AI_DEFAULT_PROVIDER === "openai") {
      ...
      (
        globalThis as unknown & { setAIProvider?: (provider: unknown) => void }
      ).setAIProvider?.(aiProvider);
      logger.info("✅ AI Provider initialized");
      logger.info("   ✅ OpenAI Provider ready\n");
    } else {
      logger.info(
        `   ℹ️  AI Provider: ${ENV_CONFIG.AI_DEFAULT_PROVIDER || "None"}\n`,
      );
    }
  } catch (error) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const databaseAdapter = container.resolve<{
        healthCheck: () => Promise<boolean>;
      }>("DatabaseAdapter");

      logger.info(
        `   📊 Database: Checking connection (Attempt ${attempt}/${MAX_RETRIES})...`,
      );
        ...
        new Promise<boolean>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  "BOOT_TIMEOUT: Database Core did not respond within 10s",
                ),
              ),
            10000,
          ),
      ...
      logger.warn(
        `   ⚠️ Database: Attempt ${attempt} failed (${errorMessage}). Retrying in ${RETRY_DELAY_MS / 1000}s...`,
      );
  ...
  if (!dbConnected) {
    throw new Error(
      "Failed to establish stable connection to Database Core after multiple attempts.",
    );
  }
