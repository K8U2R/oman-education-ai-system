...
export class ErrorTrackingService extends BaseService {
  ...
  async getErrors(_filters: any): Promise<any[]> {
    return [];
  }
  async getError(_id: string): Promise<any> {
    return { id: _id, message: "Stub Error" };
  }
  async resolveError(_id: string, _resolvedBy?: string): Promise<boolean> {
    return true;
  }
}
