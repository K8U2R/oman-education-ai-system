...
export class RedisSessionStore implements ISessionStore {
    ...
    async connect(): Promise<void> {
        try {
            ...
            logger.error('Failed to connect Redis Session Store', { error: errorMessage });
    ...
    async disconnect(): Promise<void> {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger.error('Failed to disconnect Redis Session Store', { error: errorMessage });
        }
    }
    ...
    getStore(): RedisStore {
        if (!this.store) {
            throw new Error('Redis Session Store not initialized. Call connect() first.');
        }
        return this.store;
    }
