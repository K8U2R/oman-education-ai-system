        ...
        messages: [
          {
            role: "system",
            content:
              options?.systemPrompt || "You are a helpful education assistant.",
          },
          { role: "user", content: prompt },
        ],
        ...
        messages: [
          {
            ...
            content:
              ...
              "You are a strict JSON generator. Output only valid JSON matching the schema.",
          },
          {
            role: "user",
    ...
    for (let i = 0; i < this.MAX_RETRIES; i++) {
      try {
        ...
        if (status === 429 || (status && status >= 500)) {
          ...
          logger.warn(
            `AI Provider Error (Attempt ${i + 1}/${this.MAX_RETRIES}). Retrying in ${delay}ms...`,
