...
import {
  ...
  EducationLevel,
  LessonStatus,
} from "@/domain/types/education-types.js";
  ...
  implements IEducationalRepository
{
  ...
  async getTrackById(trackId: string): Promise<EducationalTrack | null> {
    const result = await this.findOne<TrackRow>("educational_tracks", {
      id: trackId,
    });
  ...
  async getUnitsByTrackId(trackId: string): Promise<EducationalUnit[]> {
    const results = await this.findMany<UnitRow>("educational_units", {
      track_id: trackId,
    });
  ...
  async getLessonById(lessonId: string): Promise<EducationalLesson | null> {
    const result = await this.findOne<LessonRow>("educational_lessons", {
      id: lessonId,
    });
    if (!result) return null;
    return this.mapLessonToDomain(result);
