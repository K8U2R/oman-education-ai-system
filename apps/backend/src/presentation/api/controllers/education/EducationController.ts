...
export class EducationController {
  ...
  static async generateLesson(
    ...
    try {
      ...
      if (!topic || !level) {
        ...
        res
          .status(400)
          .json({ error: "Missing required fields: topic, level" });
        return;
      }
      ...
      // 3. Resolve & Execute
      const useCase = container.resolve<GenerateLessonUseCase>(
  ...
  static async getTracks(
    ...
    try {
      ...
      res
        .status(501)
        .json({ error: "Not implemented yet. Please use specific track." });
    } catch (error) {
      next(error);
    }
  }
