...
export class OAuthStateService extends EnhancedBaseService {
  ...
  constructor(
    ...
    @inject("RedisOAuthStateRepository")
    stateRepository: RedisOAuthStateRepository,
  ) {
  ...
  async validateAndGetRedirect(
    stateToken: string,
  ): Promise<{ codeVerifier: string; redirectTo: string }> {
    ...
    if (!state) {
      ...
      const { ENV_CONFIG } =
        await import("@/infrastructure/config/env.config.js");
      return {
