...
import { UserData, AuthTokens } from "../../../../domain/types/auth/index.js";
import { logger } from "../../../../shared/utils/logger.js";
import {
  InvalidTokenError,
  TokenExpiredError,
} from "../../../../domain/exceptions/AuthExceptions.js";
...
export class TokenService {
  ...
  private readonly tokenIssuer: string = "oman-education-ai";
  private readonly tokenAudience: string = "oman-education-ai-client";

  constructor(config: AuthConfig) {
    // Authenticate using the provided config object
    const accessSecret = config.jwt.accessSecret;
    const refreshSecret = config.jwt.refreshSecret;

    // Fail Fast: Ensure secrets are provided
    if (!accessSecret || !refreshSecret) {
      const errorMsg =
        "CRITICAL SECURITY ERROR: jwt.accessSecret and jwt.refreshSecret must be defined in AuthConfig.";
    ...
    logger.info("TokenService initialized with AuthConfig");
  }
  ...
  generateRefreshToken(userOrId: UserData | string, email?: string): string {
    let userId: string;
    let userEmail: string;
    let userRole: string;
