...
export class WhitelistService extends EnhancedBaseService {
  ...
  async checkStatus(email: string): Promise<{ allowed: boolean }> {
    return this.executeWithEnhancements(
      async () => {
        ...
        if (!entry) {
          logger.info("Whitelist check failed: Email not found", {
            email: cleanEmail,
          });
        ...
        if (!entry.isActive()) {
          logger.info("Whitelist check failed: Entry inactive", {
            email: cleanEmail,
          });
          return { allowed: false };
        }
        ...
        if (entry.isExpired()) {
          logger.info("Whitelist check failed: Entry expired", {
            email: cleanEmail,
          });
          return { allowed: false };
        }
        ...
        logger.info("Whitelist check passed", {
          email: cleanEmail,
          level: entry.getPermissionLevel().getValue(),
        });
        return { allowed: true };
      },
      { retryable: true },
  ...
  async getEntry(id: string): Promise<WhitelistEntryDto> {
    return this.executeWithEnhancements(
      async () => {
        ...
        const entry = await this.whitelistRepository.findById(id);
        if (!entry)
          throw new AppError("Whitelist entry not found", "NOT_FOUND", 404);
        return entry.toData();
      },
      { retryable: true },
  ...
  async deactivateEntry(id: string): Promise<WhitelistEntryDto> {
    return this.executeWithEnhancements(
      async () => {
        if (!id) throw new AppError("ID is required", "MISSING_ID", 400);
        const entry = await this.whitelistRepository.update(id, {
          is_active: false,
        });
        return entry.toData();
      },
