...
export class ChangelogService extends BaseService {
    ...
    return this.execute(async () => {
      const entries = await this.prisma.changelog_entries.findMany({
        where: {
          ...
          ...(filters.is_for_students !== undefined && {
            is_for_students: filters.is_for_students,
          }),
          ...(filters.is_for_teachers !== undefined && {
            is_for_teachers: filters.is_for_teachers,
          }),
          ...(filters.is_for_developers !== undefined && {
            is_for_developers: filters.is_for_developers,
          }),
        },
        orderBy: {
          published_at: "desc",
  ...
  private summarizeReactions(
    feedback: Array<{ reaction_type: string | null }> | null,
