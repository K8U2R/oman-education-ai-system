...
import { LessonVideos } from '@/application/features/learning/types'
...
export const LessonVideosTab: React.FC<LessonVideosTabProps> = ({ loading, videos }) => {
  ...
  if (loading) {
    return (
      <div className="lesson-detail-page__content-loading">
        ...
        <p className="lesson-detail-page__content-text">{t('lesson.videos.searching')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="lesson-detail-page__content-loading">
      <p className="lesson-detail-page__content-text">{t('lesson.videos.load_prompt')}</p>
    </div>
  )
}
