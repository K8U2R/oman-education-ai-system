...
class BackgroundSyncService {
  ...
  private saveSyncTasks(): void {
    try {
      enhancedCacheService.set('sync_tasks', this.syncTasks, {
   ...
   * تحميل مهام المزامنة
   */
  async loadSyncTasks(): Promise<void> {
    try {
      const stored = await enhancedCacheService.get<SyncTask[]>('sync_tasks', { useIndexedDB: true })
  ...
  constructor() {
    this.loadSyncTasks().catch(err => console.error('Failed to init sync tasks', err))
  }
