...
import { enhancedCacheService } from '../persistence/enhanced-cache.service'
...
class OfflineService {
  ...
  private savePendingRequests(): void {
    try {
      // The original instruction had a syntax error and implied a change in logic.
      // Adhering to the instruction "Replace cacheService usage with enhancedCacheService"
      // while maintaining the original logic of saving the entire array.
      enhancedCacheService.set('offline_requests', this.pendingRequests, {
  ...
  private async loadPendingRequests(): Promise<void> {
    try {
      const stored = await enhancedCacheService.get<OfflineRequest[]>('offline_requests', { useIndexedDB: true })
