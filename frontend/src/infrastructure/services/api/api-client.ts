...
class ApiClient {
  ...
  private redirectToLogin(): void {
    if (typeof window !== 'undefined') {
      console.warn('[API Client] Auto-redirect DISABLED for debugging (Auth Freeze Mode).')
      // // مسح tokens من جميع المصادر
      // tokenManager.clearTokens()

      // // مسح auth store
      // useAuthStore.getState().logout()

      // // إعادة التوجيه بدون إعادة تحميل الصفحة (إذا كان React Router متاحاً)
      // // إذا لم يكن متاحاً، نستخدم window.location كحل احتياطي
      // if (window.location.pathname !== '/login') {
      //   // استخدام replaceState لتجنب إضافة entry جديد في history
      //   window.history.replaceState({}, '', '/login')
      //   // إطلاق حدث مخصص لإعلام React Router بالتغيير
