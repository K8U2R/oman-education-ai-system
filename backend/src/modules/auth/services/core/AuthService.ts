        ...
        return this.executeWithEnhancements(
            async () => {
                ...
                try {
                    ...
                    logger.error("Failed to send verification email during registration", { userId: user.id, error: emailError });
                ...
                const tokens = this.tokenService.generateTokens(user as unknown as UserData);
    ...
    async sendVerificationEmail(email: string): Promise<void> {
        return this.executeWithEnhancements(
            async () => {
                ...
                await this.emailService.sendVerificationEmail(email, token, user.firstName);
                logger.info("Verification token created and email sent", { userId: user.id });
            },
            { retryable: true },
    ...
    async requestPasswordReset(email: string): Promise<void> {
        return this.executeWithEnhancements(
            async () => {
                ...
                await this.emailService.sendPasswordResetEmail(email, token, user.firstName);
