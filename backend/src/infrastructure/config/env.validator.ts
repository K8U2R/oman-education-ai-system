...
export function validateEnvironment(): void {
    ...
    if (ENV_CONFIG.NODE_ENV === 'production') {
        ...
        if (ENV_CONFIG.FRONTEND_URL && !ENV_CONFIG.FRONTEND_URL.startsWith('https://')) {
    ...
    if (ENV_CONFIG.NODE_ENV === 'development') {
        ...
        if (ENV_CONFIG.FRONTEND_URL?.includes('k8u2r.online')) {
            warnings.push('FRONTEND_URL appears to be a production URL in development mode');
        }

        if (ENV_CONFIG.APP_URL?.includes('k8u2r.online')) {
            warnings.push('APP_URL appears to be a production URL in development mode');
        }
    }
