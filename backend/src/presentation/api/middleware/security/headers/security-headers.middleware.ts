...
function buildCSPHeader(directives: Record<string, string[]>, upgradeInsecure: boolean): string {
 ...
 */
export function securityHeadersMiddleware(config: SecurityHeadersConfig = defaultConfig) {
    return (req: Request, res: Response, next: NextFunction): void => {
        try {
            ...
            if (finalConfig.contentSecurityPolicy?.enabled) {
                ...
                res.setHeader("Content-Security-Policy", buildCSPHeader(directives, !!finalConfig.contentSecurityPolicy.upgradeInsecureRequests));
            }

            // HSTS (Only Production + HTTPS)
            if (finalConfig.strictTransportSecurity?.enabled && (req.secure || req.headers["x-forwarded-proto"] === "https")) {
            ...
            if (finalConfig.xFrameOptions?.enabled) res.setHeader("X-Frame-Options", finalConfig.xFrameOptions.value || "DENY");
            if (finalConfig.xContentTypeOptions?.enabled) res.setHeader("X-Content-Type-Options", "nosniff");
            if (finalConfig.referrerPolicy?.enabled) res.setHeader("Referrer-Policy", finalConfig.referrerPolicy.value || "strict-origin");

            // Permissions
            if (finalConfig.permissionsPolicy?.enabled && finalConfig.permissionsPolicy.features) {
                res.setHeader("Permissions-Policy", buildPermissionsPolicyHeader(finalConfig.permissionsPolicy.features));
            }
    ...
    return securityHeadersMiddleware({
        contentSecurityPolicy: {
            ...
            directives: {
                ...
                "style-src": ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com"],
                "img-src": ["'self'", "data:", "blob:", "https://*.googleusercontent.com"],
                "connect-src": ["'self'", "https://api.openai.com", "https://api.anthropic.com", "http://localhost:*", "ws://localhost:*"],
                "font-src": ["'self'", "https://fonts.gstatic.com", "data:"],
                "object-src": ["'none'"],
                "frame-ancestors": ["'self'"],
            },
