/**
 * Auth Handlers - معالجات المصادقة
 *
...
} from "@/presentation/api/dto/auth/auth.dto.js";
...
export class AuthHandler extends BaseHandler {
  ...
  handleOAuthCallback = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Assuming frontend login page is at /login or similar, or we act generic
      // We'll redirect to a generic error page or back to login
      const errorMsg =
        error instanceof Error ? error.message : "Authentication failed";
      // Use a known frontend URL or derive it?
      // Safe fallback: redirect to origin if possible, or assume localhost:5173 for dev
