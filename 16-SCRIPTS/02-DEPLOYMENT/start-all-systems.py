...
def start_operating_system() -> Optional[subprocess.Popen]:
    ...
    os_path = project_root / "01-OPERATING-SYSTEM" / "api_gateway" / "fastapi_server.py"
    ...
    try:
        process = subprocess.Popen(
            [sys.executable, "-m", "api_gateway.fastapi_server"],
            cwd=str(project_root / "01-OPERATING-SYSTEM"),
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
        print("✅ تم تشغيل نظام التشغيل على http://localhost:8001")
        return process
...
def start_integration_system() -> Optional[subprocess.Popen]:
    ...
    try:
        process = subprocess.Popen(
            [sys.executable, "-m", "uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8003", "--reload"],
            cwd=str(project_root / "02-SYSTEM-INTEGRATION" / "integration-orchestrator"),
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
        print("✅ تم تشغيل نظام التكامل على http://localhost:8003")
        return process
    except Exception as e:
